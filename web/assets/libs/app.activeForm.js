...
(function ($) {
    ...
    var defaults = {
        ...
        // whether to save date after validate successful
                ...
                if ($.isEmptyObject(messages) && needAjaxValidation) {
                    ...
                    $.ajax({
                            ...
                            if (msgs !== null && typeof msgs === 'object') {
                                if (data.settings.validateAndSave && msgs.status) {
                                    if (msgs.status === 'error' && msgs.message) {
                                        ...
                                        showMessages($form, msgs.message, '错误', null, 'error');
                                    } else if (msgs.status === 'success') {
                                        if (msgs.message) {
                                            showMessages($form, msgs.message, '信息', function () {
                                                if (msgs.redirectUrl) {
                                        ...
                                        showMessages($form, msgs.status, '信息', null, 'info');
                                    }
                                } else {
                                    $.each(data.attributes, function () {
                                ...
                                if (typeof msgs === 'string' && msgs.length > 0) {
                                    showMessages($form, msgs, '信息', null, 'info');
                                } else {
                        ...
                        error: function (jqXHR, status, error) {
                            submitFinalize($form);
                            showMessages($form, ['内容：' + jqXHR.responseText, '错误：' + error], '错误', null, 'error');
                        }
                    });
        ...
        if (submitting) {
            ...
            if (errorAttributes.length) {
                if (data.settings.messageDisplayMode != MESSAGE_DISPLAY_MODE_SIMPLE) {
                    showMessages($form, messages, '错误', null, 'error');
                }
    ...
    var updateSummary = function ($form, messages) {
        var data = $form.data('yiiActiveForm');
        var $summary = $form.find(data.settings.errorSummary),
            $ul = $summary.find('ul').empty();

        if ($summary.length && messages) {
            $ul.append(buildMessageList(messages).html());
     ...
     * @returns {*|HTMLElement}
     */
    var buildMessageList = function ($form, messages) {
        var settings = $form.data('yiiActiveForm').settings;
    ...
    var showMessages = function ($form, messages, title, callback, msgType) {
        ...
        if (messages) {
            if (settings.messageDisplayMode === MESSAGE_DISPLAY_MODE_SUMMARY) {
                ...
                var message = buildMessageList($form, messages);
                message = $('<div />').append(message).html();
                title = title || '错误';
                if ($.dialog) {
                    msgType = msgType || 'error';
                    $.dialog.setState(msgType).messageBox(message, title, true, callback);
                } else if ($.weui) {
